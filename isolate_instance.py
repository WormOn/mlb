...
def apps_list():
    ...
    with open('/root/haproxy.cfg','r+') as fd:
...
def get_acl():
    ...
    with open("/root/haproxy.cfg","r+") as fd:
        for line in fd.readlines():
            acl_r=re.search(r'acl',line)
            if acl_r:
                acl=str(line).split()[2]+' '+str(line).split()[3]+'\n'
                print (acl)
                with open('haproxy_acl','a+') as facl:
                    facl.write(acl)
...
def isolate_instance(instance):
    ...
    with open('/root/haproxy.cfg','r+') as fd:
        for line in fd.readlines():
            ...
            with open('/root/haproxy.cfg.tmp','a+') as ftmp:
                if isolate_r:
    ...
    os.system('rm -rf /root/haproxy.cfg')
    os.rename('/root/haproxy.cfg.tmp','/root/haproxy.cfg')
    return result
...
if __name__=='__main__':
    print("Test Module...")
    #tag_instance('192.168.1.14:31689')
